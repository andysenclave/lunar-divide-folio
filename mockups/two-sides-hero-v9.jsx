...
const ThemeContext = createContext({ theme: 'dark', colors: THEMES.dark, toggleTheme: () => {} });
  ...
  return (
    <div style={{ 
      ...
      <motion.div 
        ...
        animate={{
          background: lastActiveSide === 'engineering' 
      ...
      <nav style={{
        ...
        {['Engineering', 'Adventure', 'Journal'].map((item) => (
          <motion.a
            ...
            whileHover={{ y: -2 }}
            style={{
              color: activeSide === item.toLowerCase() 
                ? (item === 'Engineering' ? colors.cyan : colors.orange)
...
// ============================================
function Moon({
  moonX, moonY, moonScale, moonOpacity, moonRotation, lightX,
  scrollYProgress, isCornerMoonHovered, setIsCornerMoonHovered,
  onReturnHome, leftReveal, rightReveal, colors, theme
}) {
...
function MoonSurface({ lightX, moonRotation, theme }) {
  const moonColors = theme === 'dark' 
    ? { highlight: '#E8E8E0', mid: '#A8A8A0', dark: '#484848', shadow: '#282828' }
    : { highlight: '#F5F5F0', mid: '#D0D0C8', dark: '#787878', shadow: '#484848' };
    ...
    <div style={{
      ...
      <motion.div 
        style={{
          ...
          inset: 0,
          borderRadius: '50%',
          background: useTransform(lightX, (x) => `
      ...
      <motion.div 
        style={{
          ...
          inset: 0,
          borderRadius: '50%',
          background: useTransform(moonRotation, (r) => `
      ...
      <div style={{
        ...
        inset: 0,
        borderRadius: '50%',
        boxShadow: 'inset -30px -20px 60px rgba(0,0,0,0.5), inset 10px 10px 40px rgba(255,255,255,0.05)',
      }} />
    </div>
  );
}
...
// PORTRAIT OVERLAYS
// ============================================
function PortraitOverlays({ leftReveal, rightReveal, scrollYProgress, colors }) {
  ...
  return (
    ...
    >
      <motion.div style={{ position: 'absolute', inset: 0, opacity: leftReveal }}>
        <EngineeringPortrait colors={colors} />
      </motion.div>
      
      <motion.div style={{ position: 'absolute', inset: 0, opacity: rightReveal }}>
...
function EngineeringPortrait({ colors }) {
  return (
    <div style={{
      ...
      height: '100%',
      borderRadius: '50%',
      background: 'linear-gradient(145deg, #0a1628 0%, #0d1f35 50%, #051018 100%)',
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'center',
      position: 'relative',
...
function AdventurePortrait({ colors }) {
  return (
    <div style={{
      ...
      height: '100%',
      borderRadius: '50%',
      background: 'linear-gradient(145deg, #1a1005 0%, #2a1a0a 50%, #100800 100%)',
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'center',
      position: 'relative',
      ...
      <div style={{
        ...
        clipPath: 'polygon(0% 100%, 15% 60%, 30% 80%, 45% 40%, 55% 55%, 70% 25%, 85% 50%, 100% 30%, 100% 100%)',
      }} />
...
function Tagline({ activeSide, opacity, colors }) {
  const variants = {
    ...
    engineering: { tagline: 'Engineering systems.', sub: 'Building what matters.' },
    adventure: { tagline: 'Exploring worlds.', sub: 'Sharing what inspires.' },
  };
  
  const current = variants[activeSide];
  const color = activeSide === 'engineering' ? colors.cyan 
              : activeSide === 'adventure' ? colors.orange 
              : colors.text;
...
function ScrollIndicator({ lastActiveSide, opacity, colors }) {
  const color = lastActiveSide === 'engineering' ? colors.cyan
              : lastActiveSide === 'adventure' ? colors.orange
              : colors.textTertiary;
  
  const text = lastActiveSide === 'engineering' ? 'Scroll into Engineering'
             : lastActiveSide === 'adventure' ? 'Scroll into Adventure'
             : 'Scroll to explore';
...
function EngineeringSection({ colors }) {
  ...
  const skills = [
    { title: 'Frontend Architecture', skills: ['React', 'Next.js', 'TypeScript', 'Redux'], icon: '◧' },
    { title: 'Backend & APIs', skills: ['Node.js', 'GraphQL', 'REST', 'Microservices'], icon: '◨' },
    { title: 'DevOps & Cloud', skills: ['AWS', 'Docker', 'CI/CD', 'Kubernetes'], icon: '◩' },
    { title: 'Leadership', skills: ['Team Building', 'Architecture', 'Mentoring', 'Agile'], icon: '◪' },
  ];

  const projects = [
    { title: 'Enterprise Rating Platform', desc: 'Led team of 8 engineers to deliver financial ratings system', tech: 'React, Node.js, PostgreSQL' },
    { title: 'E-commerce Microservices', desc: 'Architected scalable backend handling 10K+ concurrent users', tech: 'GraphQL, AWS, Docker' },
    { title: 'Real-time Analytics Dashboard', desc: 'Built data visualization platform with live updates', tech: 'Next.js, D3.js, WebSockets' },
  ];
  ...
  return (
    <motion.section 
      ref={ref}
      id="engineering" 
      style={{ minHeight: '100vh', padding: 'clamp(80px, 10vh, 120px) clamp(24px, 5vw, 48px)', position: 'relative' }}
    >
      <motion.div style={{ maxWidth: 1200, margin: '0 auto', opacity, y }}>
        ...
        <div style={{
          ...
          {skills.map((category, i) => (
            ...
            >
              <div style={{ fontSize: 24, marginBottom: 16, color: colors.cyan }}>{category.icon}</div>
              <h3 style={{ color: colors.text, fontSize: 18, fontWeight: 600, marginBottom: 16 }}>{category.title}</h3>
        ...
        <div>
          <h3 style={{ color: colors.cyan, fontSize: 14, fontWeight: 600, letterSpacing: '0.2em', marginBottom: 32 }}>
            SELECTED PROJECTS
          </h3>
          {projects.map((project, i) => (
            ...
            >
              <div>
                <h4 style={{ color: colors.text, fontSize: 18, fontWeight: 500, marginBottom: 8 }}>{project.title}</h4>
                <p style={{ color: colors.textSecondary, fontSize: 14 }}>{project.desc}</p>
              </div>
              <span style={{ color: `${colors.cyan}b3`, fontSize: 12, fontFamily: 'monospace' }}>{project.tech}</span>
            </motion.div>
          ))}
        </div>
      </motion.div>
...
function AdventureSection({ colors }) {
  ...
  const content = [
    { title: 'Travel Vlogs', desc: 'Documenting journeys across continents', count: '12 videos', icon: '🎬' },
    { title: 'Audio Stories', desc: 'Narratives from the road less traveled', count: '8 episodes', icon: '🎧' },
    { title: 'Photography', desc: 'Capturing moments that matter', count: '200+ shots', icon: '📷' },
  ];
  ...
  return (
    ...
    >
      <motion.div style={{ maxWidth: 1200, margin: '0 auto', opacity, y }}>
        <motion.div style={{ marginBottom: 'clamp(40px, 8vh, 80px)', textAlign: 'right' }}>
          <span style={{
            color: colors.orange,
            fontSize: 12,
            fontWeight: 600,
        ...
        <div style={{
          ...
          {content.map((item, i) => (
            ...
            >
              ...
              <h3 style={{ color: colors.text, fontSize: 20, fontWeight: 600, marginBottom: 12 }}>{item.title}</h3>
              <p style={{ color: colors.textSecondary, fontSize: 14, marginBottom: 16, lineHeight: 1.6 }}>{item.desc}</p>
              <span style={{ color: colors.orange, fontSize: 12, fontWeight: 600 }}>{item.count}</span>
            </motion.div>
          ))}
        </div>
        ...
        >
          <span style={{ color: colors.orange, fontSize: 11, fontWeight: 600, letterSpacing: '0.2em' }}>FEATURED JOURNEY</span>
          <h3 style={{ color: colors.text, fontSize: 'clamp(22px, 3vw, 28px)', fontWeight: 600, margin: '16px 0' }}>Skydiving in Norway</h3>
          <p style={{ color: colors.textSecondary, fontSize: 16, lineHeight: 1.7, maxWidth: 600 }}>
            15,000 feet above the Norwegian fjords. The moment before the jump — 
            where engineering precision meets pure adventure. That's where I feel most alive.
          </p>
          <motion.a 
            href="#" 
            whileHover={{ x: 4 }}
            style={{ display: 'inline-block', marginTop: 24, color: colors.orange, textDecoration: 'none', fontSize: 14, fontWeight: 500 }}
          >
            Watch the vlog →
          </motion.a>
        </motion.div>
...
function JournalSection({ colors }) {
  const posts = [
    { title: 'On Building vs. Exploring', date: 'Dec 2024', tags: ['thoughts', 'balance'] },
    { title: 'Lessons from Leading Remote Teams', date: 'Nov 2024', tags: ['engineering', 'leadership'] },
    { title: 'Why I Started Creating Content', date: 'Oct 2024', tags: ['story', 'journey'] },
  ];

  return (
    <section id="journal" style={{ minHeight: '60vh', padding: 'clamp(80px, 10vh, 120px) clamp(24px, 5vw, 48px)' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        >
          <span style={{ color: colors.textSecondary, fontSize: 12, fontWeight: 600, letterSpacing: '0.3em', textTransform: 'uppercase' }}>
            Journal
          </span>
          <h2 style={{ fontSize: 'clamp(28px, 4vw, 48px)', fontWeight: 700, color: colors.text, marginTop: 16 }}>
            Thoughts & Writings
          </h2>
        </motion.div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 32 }}>
          {posts.map((post, i) => (
            ...
            >
              <span style={{ color: colors.textTertiary, fontSize: 12 }}>{post.date}</span>
              <h3 style={{ color: colors.text, fontSize: 18, fontWeight: 500, margin: '12px 0 16px', lineHeight: 1.4 }}>{post.title}</h3>
              <div style={{ display: 'flex', gap: 8 }}>
                {post.tags.map((tag, j) => (
                  <span key={j} style={{ color: colors.textSecondary, fontSize: 11, padding: '4px 8px', background: colors.bgHover, borderRadius: 4 }}>#{tag}</span>
                ))}
              </div>
            </motion.article>
          ))}
...
function ContactSection({ colors }) {
  return (
    <section id="contact" style={{ padding: 'clamp(80px, 10vh, 120px) clamp(24px, 5vw, 48px)' }}>
      <motion.div 
        initial={{ opacity: 0, y: 30 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
      ...
      >
        ...
        <h2 style={{ fontSize: 'clamp(28px, 4vw, 48px)', fontWeight: 700, color: colors.text, marginTop: 16, marginBottom: 24 }}>
          Let's Build Something<br />
          <span style={{ background: `linear-gradient(90deg, ${colors.cyan}, ${colors.orange})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Together</span>
        </h2>
        <p style={{ color: colors.textSecondary, fontSize: 16, lineHeight: 1.7, marginBottom: 40, maxWidth: 500, margin: '0 auto 40px' }}>
          Whether it's a technical challenge, a collaboration opportunity, or just a conversation — I'd love to hear from you.
        </p>
        <motion.a 
...
function Footer({ colors }) {
  return (
    <footer style={{ 
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 48, marginBottom: 60 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: colors.text, fontSize: 16, fontWeight: 600, marginBottom: 16 }}>
              <span style={{ fontSize: 20, opacity: 0.7 }}>☽</span>
              ANINDYA
            </div>
            <p style={{ color: colors.textSecondary, fontSize: 14, maxWidth: 300, lineHeight: 1.6 }}>
              Senior Technical Lead by profession.<br />Content creator by passion.
            </p>
          </div>
          <div>
            <h4 style={{ color: colors.textTertiary, fontSize: 11, fontWeight: 600, letterSpacing: '0.15em', marginBottom: 16 }}>CONNECT</h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {['GitHub', 'LinkedIn', 'YouTube', 'Twitter'].map((link, i) => (
                <motion.a 
                  ...
                  style={{ color: colors.textSecondary, textDecoration: 'none', fontSize: 14, transition: 'all 0.2s ease' }}
                >
                  {link}
                </motion.a>
              ))}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 24, borderTop: `1px solid ${colors.divider}`, flexWrap: 'wrap', gap: 16 }}>
          <span style={{ color: colors.textTertiary, fontSize: 12 }}>© 2025 andysenclave.com</span>
          <span style={{ color: colors.textDisabled, fontSize: 11, fontStyle: 'italic', maxWidth: 400, textAlign: 'right' }}>
            The moon rotates once every 27.3 days — the same time it takes to orbit Earth. That's why we only ever see one side from here.
          </span>
        </div>
      </div>
    </footer>
